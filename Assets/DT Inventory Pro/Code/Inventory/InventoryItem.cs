...
namespace DTInventory
{
    ...
    public class InventoryItem : MonoBehaviour, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        /// <summary>
        /// Reference to EquipmentPanel to check if item was equipped somewhere
        /// </summary>
        ...
        void Update()
        {
            if (drag)
            {
                if (inventory.disableItemDragInterpolation)
                {
                    m_rect.pivot = new Vector2(0.5f, 0.5f);
                    m_rect.position = dragEventData.position;
                }
                else
                {
            ...
            else
            {
                if (inventory.disableItemDragInterpolation)
                {
                    ...
                    m_rect.anchoredPosition = finalPosition;
                }
                else
                {
        ...
        //Inventory Tooltip implementation
        public void OnPointerEnter(PointerEventData eventData)
        {
            if(Tooltip.instance != null)
            {
                StartCoroutine(ShowTooltip(eventData));
            }
        }
        ...
        public IEnumerator ShowTooltip(PointerEventData eventData)
        {
            yield return new WaitForSeconds(0.1f);

            Tooltip.GenerateContent(item.title, item.description, eventData.position);
        }
    }
}
