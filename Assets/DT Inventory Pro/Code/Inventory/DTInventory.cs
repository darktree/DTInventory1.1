...
namespace DTInventory
{
    ...
    public class DTInventory : MonoBehaviour
    {
        ...
        /// <summary>
        /// The distance item will dropped of with
        /// </summary>
        ...
        /// <summary>
        /// Value to calculate total items weight
        /// </summary>
        public float totalItemsWeight { get { return TotalItemsWeight(); } }
        ...
        public bool disableItemDragInterpolation = true;
        ...
        public bool AddItem(Item item, int x, int y)
        {
            ...
            else
            {
                if (autoEquipItems)
                {
                    foreach (var equipmentPanel in equipmentPanels)
                    {
                        if (equipmentPanel.allowedItemType == item.type && equipmentPanel.equipedItem == null)
                        {
                            var _InventoryItem = Instantiate(cell).gameObject.AddComponent<InventoryItem>();
                            var _InventoryItemImage = _InventoryItem.GetComponent<Image>();
        ...
        /// <returns></returns>
        public virtual bool CheckIfItemExist(string itemTitle)
        {
        ...
        public virtual void EquipItem(EquipmentPanel panel, InventoryItem item)
        {
            item.transform.SetParent(panel.mainSlot.transform.parent);

            MarkSlots(item.x, item.y, item.width, item.height, true);
        ...
        public virtual void MarkSlots(int startSlot_x, int startSlot_y, int width, int height, bool isFree)
        {
        ...
        public GridSlot CheckFreeSpaceAtSlot(int cell_x, int cell_y, int width, int height)
        {
            for (int i = cell_x; i < cell_x + width; i++)
            {
                ...
                {
                    if (FindSlotByIndex(i, j) == null || FindSlotByIndex(i, j).isLoot || FindSlotByIndex(i, j).free == false)
                    {
                        return null;
                    }
                }
        ...
        public GridSlot CheckFreeSpaceAtSlotLoot(int cell_x, int cell_y, int width, int height)
        {
            for (int i = cell_x; i < cell_x + width; i++)
            {
                for (int j = cell_y; j < cell_y + height; j++)
        ...
        /// </summary>
        public virtual void DrawRegularSlotsColors()
        {
        ...
        public virtual void DrawColorsForHoveredSlots(int startSlot_x, int startSlot_y, int width, int height)
        {
            for (int i = startSlot_x; i < startSlot_x + width; i++)
            {
                for (int j = startSlot_y; j < startSlot_y + height; j++)
        ...
        /// <param name="width">Item width we want to put to this slot</param>
        /// <param name="height">Item height we want to put to this slot</param>
        public virtual void DrawColorForStackableHoveredSlots(int startSlot_x, int startSlot_y, int width, int height)
        {
            for (int i = startSlot_x; i < startSlot_x + width; i++)
            {
                for (int j = startSlot_y; j < startSlot_y + height; j++)
        ...
        public GridSlot CheckFreeSpaceForAllSlots(int width, int height)
        {
            foreach (var slot in slots)
            {
                if (CheckFreeSpaceAtSlot(slot.x, slot.y, width, height) != null && slot.free && slot.equipmentPanel == null)
        ...
        {
            print("Adding items to loot grid area");

            if (items == null)
            {
                print("Items are null return");
                return;
            }
            ...
            foreach (var _item in items)
            {
                if (CheckFreeSpaceForAllSlotsLoot(_item.width, _item.height))
                {
                    ...
                    print("Adding " + _item + " to - " + slotToEquip.x + "|" + slotToEquip.y);
                }
                else
                {
        ...
        public GridSlot CheckFreeSpaceForAllSlotsLoot(int width, int height)
        {
            foreach (var slot in slots)
            {
                if (CheckFreeSpaceAtSlotLoot(slot.x, slot.y, width, height) && slot.equipmentPanel == null && slot.isLoot == true)
                    return CheckFreeSpaceAtSlotLoot(slot.x, slot.y, width, height);
            }
        ...
        /// <summary>
        /// Method to find existing inventory item by it's InventoryItem reference (Not by Item.cs!)
        /// </summary>
        /// <param name="item">InventoryItem to find</param>
        /// <returns>InventoryItem if found - null if not</returns>
        public InventoryItem FindInventoryItemHandlerByItem(Item item)
        {
        ...
        public void SwapEquipment(InventoryItem InventoryItem, bool closeInventory)
        {
            foreach (var panel in equipmentPanels)
            {
                if (panel.allowedItemType == InventoryItem.item.type && panel.equipedItem == null)
                {
                    //Mark existing item position as free
                    MarkSlots(InventoryItem.x, InventoryItem.y, InventoryItem.item.width, InventoryItem.item.height, true);
                    EquipItem(panel, InventoryItem);
                    InventoryItem.equipmentPanel = panel;
                    break;
                }
                else if (panel.allowedItemType == InventoryItem.item.type && panel.equipedItem != null)
                {
                    //If we have enough place to store equiped item
                    if (CheckFreeSpaceForAllSlots(panel.equipedItem.width, panel.equipedItem.height))
                    {
                        UnequipItemFromPanel(panel);
